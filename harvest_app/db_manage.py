...
def insert_links(user, password, host, dbname, link):
    try:
        ...
        # connection.set_isolation_level(ISOLATION_LEVEL_AUTOCOMMIT)
        cursor = connection.cursor()
        insert_query = "INSERT INTO downloadlinks (Link) VALUES(%s)"
...
def select_details(user, password, host, dbname, detail_dict):
    try:
        ...
        connection = establish_connection(user, password, host, dbname)
        cursor = connection.cursor()
        search_query = "SELECT * FROM downloadpages WHERE Name='{}' and Version ='{}'".format\
            (detail_dict["name"],detail_dict["version"])
        cursor.execute(search_query)
        result = cursor.fetchone()
        if result is None:
            return True
...
def select_translations(user, password, host, dbname, translation_dict):
    try:
        ...
        connection = establish_connection(user, password, host, dbname)
        cursor = connection.cursor()
        search_query = "SELECT * FROM downloadtranslations " \
                       "WHERE Link='{}' and Version ='{}'".format\
            (translation_dict["language"], translation_dict["version"])
        cursor.execute(search_query)
        result = cursor.fetchone()
        if result is None:
            return True
